...
class DateTimeModel(BaseModel):
    ...
    date: str = Field(
        ...,
        description="Propertly formatted date",
...
class DateModel(BaseModel):
    ...
    date: str = Field(
        ..., description="Properly formatted date", pattern=r"^\d{4}-\d{2}-\d{2}$"
    )
...
class IdentificationNumberModel(BaseModel):
    ...
    id: int = Field(
        ..., description="identification number without dots", pattern=r"^\d{7,8}$"
    )
